import { TimezoneException, DateException } from "./exceptions.js"
...
function getTimeFromTimezone(timezone: Timezone, currentDate = new Date()) {
    ...
    utc.setTime(utc.getTime() + (Number(timezone) * 60 * 60 * 1000))
...
function convertHMTzToHTz(hmtimezone: HMTimezone) {
    if (!isHMtimezone(hmtimezone) || !hmtimezone) throw new TimezoneException("This isn't a HH:MM timezone format", constants.ExceptionCodes.Timezones.NOT_HMTZ_FORMAT)

    //@ts-ignore
    const [hours, minutes]: number[] = hmtimezone.match(/[^:]*/g).filter(s => Boolean(s))

    const minsConvertedToHours: number = Number(minutes) / 60
...
function formatDate(date: Date, timezone?: Timezone, isAMPMFormat = false) {
...
function ensureTimezone(timezone: any) {
...
function addIndicatorsToTimezone(timezone: any) {
    timezone = String(timezone)

    if (timezone.charAt(0) !== "+" && timezone.charAt(0) !== "-" && Number(timezone) !== 0) timezone = "+" + timezone
